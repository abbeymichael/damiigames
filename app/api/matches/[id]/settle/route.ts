  ...
  try {
    ...
    if (!auth || !auth.user) {
      return NextResponse.json(
        { success: false, error: "Unauthorized. Valid session required to settle match." },
        { status: 401 }
      );
    }
    ...
    const safeWinnerId = securityService.sanitizeInput(String(winnerId));
    const matchData = await dbRepository.getMatch(safeMatchId);
    if (!matchData) {
      return NextResponse.json({ success: false, error: "Match not found" }, { status: 404 });
    }

    // Authorization: Must be one of the participants or an admin/super_admin
    ...
    if (!isSuperAdmin && !isAdmin && !isParticipant) {
      return NextResponse.json(
        { success: false, error: "Forbidden: You are not authorized to settle this match." },
        { status: 403 }
      );
    }

    const match = await ledgerService.settleMatchEscrow(safeMatchId, safeWinnerId);
