...
export function rowToProfile(row: ProfileRow): Profile {
  return {
    ...
    mfaEnrolledAt: orUndefined(row.mfaEnrolledAt),
    mfaPreferredMethod: (row.mfaPreferredMethod as any) || undefined,
    totpSecret: orUndefined(row.totpSecret),
    totpEnabled: Boolean(row.totpEnabled),
    totpVerifiedAt: orUndefined(row.totpVerifiedAt),
    passkeys: parseJson<UserPasskey[]>(row.passkeysJson, []),
    backupCodes: parseJson<string[]>(row.backupCodesJson, []),
...
export function profileToRow(p: Profile): ProfileRow {
  return {
    ...
    mfaEnrolledAt: orNull(p.mfaEnrolledAt),
    mfaPreferredMethod: clamp(p.mfaPreferredMethod, 32),
    totpSecret: clamp(p.totpSecret, 128),
    totpEnabled: p.totpEnabled ? 1 : 0,
    totpVerifiedAt: orNull(p.totpVerifiedAt),
...
export function rowToRoom(row: RoomRow): Room {
  ...
  const room: Room = {
    ...
    hostReady: toBool(row.hostReady),
    guestReady: toBool(row.guestReady),
...
export function roomToRow(r: Room): RoomRow {
  ...
  return {
    ...
    isPrivate: fromBool(r.isPrivate),
    hostReady: fromBool(r.hostReady),
    guestReady: fromBool(r.guestReady),
