...
export const MECHANICS_FUND_ACCOUNT_ID = "mechanics-treasury";

export function isMechanicOrBotUser(userId?: string, referenceType?: string): boolean {
  if (!userId) return false;
  return (
    userId === MECHANICS_FUND_ACCOUNT_ID ||
    userId === "mechanics-fund" ||
    userId.startsWith("mech-") ||
    userId.startsWith("mechanic-") ||
    userId.startsWith("bot-") ||
    userId.startsWith("bot_") ||
    referenceType === "paystack_bot_funding" ||
    referenceType === "paystack_bulk_bot_funding" ||
    referenceType === "system_bot_funding" ||
    referenceType === "system_bot_reclaim" ||
    referenceType === "mechanic_bankroll_funding" ||
    referenceType === "bot_funding" ||
    referenceType === "bot_withdrawal" ||
    referenceType === "admin_bot_allocation"
  );
}
...
export function determineFundType(
  ...
  if (accountType === "escrow" || entryType?.includes("escrow") || entryType?.includes("lock")) {
    return "escrow";
  }
  if (isMechanicOrBotUser(userId, referenceType) && userId !== PLATFORM_ACCOUNT_ID && entryType !== "platform_fee") {
    return "mechanics_fund";
  }
...
export const CANONICAL_CHART_OF_ACCOUNTS: Array<{
  ...
  {
    code: "1040",
    name: "Mechanics Operating Float (Bot Bankrolls)",
    accountClass: "asset",
    fundType: "mechanics_fund",
    normalBalance: "debit",
    description: "Liquid bankroll capital actively deployed across AI mechanic/bot player wallets for automated matchmaking and gameplay.",
  },
  {
    code: "1045",
    name: "Mechanics Reserve Vault (Treasury Clearing)",
    accountClass: "asset",
    fundType: "mechanics_fund",
    normalBalance: "debit",
    description: "Central clearing and reserve vault for AI mechanics fleet capital injections, bulk allocations, and treasury reclaims.",
  },
  {
    code: "2010",
    ...
    description: "Total platform obligation owed to registered human players on demand for cashouts or game wagers.",
  },
  {
    code: "2020",
  ...
  {
    code: "2040",
    name: "Mechanics Fleet Capital Obligations",
    accountClass: "liability",
    fundType: "mechanics_fund",
    normalBalance: "credit",
    description: "Committed platform liquidity allocated to support automated bot mechanics gameplay and wager liquidity.",
  },
  {
    code: "3010",
  ...
  {
    code: "3030",
    name: "Mechanics Accumulated Retained Reserves",
    accountClass: "equity",
    fundType: "mechanics_fund",
    normalBalance: "credit",
    description: "Cumulative net retained P&L and gameplay reserves generated by the AI mechanics fleet.",
  },
  {
    code: "4010",
  ...
  {
    code: "4040",
    name: "Mechanics Gameplay Profits (Bot Win Margin)",
    accountClass: "revenue",
    fundType: "mechanics_fund",
    normalBalance: "credit",
    description: "Wager payouts and match winnings earned by AI mechanics from human player matches.",
  },
  {
    code: "5010",
  ...
  {
    code: "5030",
    name: "Mechanics Gameplay Losses (Player Payouts)",
    accountClass: "expense",
    fundType: "mechanics_fund",
    normalBalance: "debit",
    description: "Wager payouts lost and paid out by AI mechanics to victorious human players.",
  },
  {
    code: "5040",
    name: "Mechanics Fleet Bankroll Subsidies & Maintenance",
    accountClass: "expense",
    fundType: "mechanics_fund",
    normalBalance: "debit",
    description: "Administrative capital injections and liquidity subsidies maintaining AI mechanic fleet float.",
  },
];
...
export function mapLedgerEntryToAccount(entry: {
  ...
  // 1. Mechanics / Bot specific entries
  if (entry.userId === MECHANICS_FUND_ACCOUNT_ID) {
    return { code: "1045", name: "Mechanics Reserve Vault (Treasury Clearing)", fundType: "mechanics_fund" };
  }

  if (isMechanicOrBotUser(entry.userId, entry.referenceType)) {
    if (
      ...
      entry.referenceType === "mechanic_bankroll_funding" ||
      entry.referenceType === "bot_funding" ||
      entry.referenceType === "admin_bot_allocation"
    ) {
      return { code: "1040", name: "Mechanics Operating Float (Bot Bankrolls)", fundType: "mechanics_fund" };
    }
    if (entry.referenceType === "system_bot_reclaim" || entry.referenceType === "bot_withdrawal") {
      return { code: "1045", name: "Mechanics Reserve Vault (Treasury Clearing)", fundType: "mechanics_fund" };
    }
    if ((entry.entryType === "wager_payout" && entry.accountType === "available") || entry.entryType === "mechanics_profit") {
      return { code: "4040", name: "Mechanics Gameplay Profits (Bot Win Margin)", fundType: "mechanics_fund" };
    }
    if ((entry.entryType === "wager_payout" && entry.accountType === "escrow") || entry.entryType === "mechanics_loss") {
      return { code: "5030", name: "Mechanics Gameplay Losses (Player Payouts)", fundType: "mechanics_fund" };
    }
...
/**
 * Computes live balances, inflows, outflows, and counts for the 4 System Funds:
 * 1. Account Balances Fund (Liquid Human Player Balances)
 * 2. Escrow Fund (Active Wagers & Tournament Escrows)
 * 3. Platform Fee Fund (Accumulated Platform Treasury & Commission)
 * 4. Mechanics Fund (AI Bot Fleet Bankroll Float, Clearing Vault, Profits & Losses)
 */
export async function getSystemFundsReport(): Promise<SystemFundsReport> {
  ...
  for (const entry of allEntries) {
    ...
    const fund = determineFundType(
      entry.userId,
      entry.accountType as LedgerAccountType,
      entry.entryType,
      entry.referenceType,
      entry.referenceId
    );
    ...
    if (fund === "platform_fee") {
      ...
      if (amt >= 0) {
        mechanicsInflow += amt;
        if (entry.entryType === "wager_payout" || entry.entryType === "mechanics_profit") {
          mechanicsProfits += amt;
        }
      } else {
        mechanicsOutflow += Math.abs(amt);
        if (entry.entryType === "wager_payout" || entry.entryType === "mechanics_loss") {
...
/**
 * Performs a deep reconciliation audit across all 4 funds and individual ledger accounts.
 */
...
/**
 * Returns Mechanics Fund analytics, operating float, clearing vault, and bot PnL.
 */
export async function getMechanicsFundDetails(): Promise<MechanicsFundDetails> {
  const fundsReport = await getSystemFundsReport();
  const coaReport = await getChartOfAccountsReport();
  ...
  const mechanicsEntries = allEntries
    .filter((e) => {
      const fund = determineFundType(
        e.userId,
        e.accountType as LedgerAccountType,
        e.entryType,
        e.referenceType,
        e.referenceId
      );
  ...
  return {
    ...
    recentMechanicsEntries: mechanicsEntries,
    lastUpdated: new Date().toISOString(),
  };
}
